...
def main(
    ...
    if performance:
        ...
        if eval_only == False:
            ...
            _, _ = evaluate(
                model, test_images, test_labels, save_path=f"saved_models/{model_name}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        choices=["AlexNet", "TinyCNN", "OIANet", "ResNet18"],
        default="OIANet",
    ...
    parser.add_argument(
        "--performance", action="store_true", help="Enable performance measurement"
    )
    parser.add_argument(
        "--eval_only", action="store_true", help="Enable evaluation-only mode"
    )
    parser.add_argument(
        "--conv_algo",
        type=int,
    ...
    conv_algo = (
        args.conv_algo
    )  # PISTA: esto sirve para seleccionar nuevos algoritmos de convolucion
    dense_algo = (
        args.dense_algo
