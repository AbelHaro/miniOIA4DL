...
def test_conv2d():
    ...
    for conv_algo in [0, 1, 2]:
            ...
            for c in range(out_channels):
                for i in range(out_h):
                    for j in range(out_w):
                        ...
                        expected_output[b, c, i, j] = np.sum(
                            patch
                        )  # kernel is all ones
        ...
        assert np.allclose(output, expected_output), (
            f"Conv2D (conv_algo={conv_algo}) forward mismatch!"
        )
        print(f"✅ Conv2D forward (algo={conv_algo}) with padding and stride passed")
